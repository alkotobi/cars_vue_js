-- ============================================
-- Migration: Convert sell_bill notes column to JSON
-- ============================================
...
SET @sql = IF(@col_exists = 0, 
  'ALTER TABLE `sell_bill` ADD COLUMN `notes_json` JSON DEFAULT NULL COMMENT ''JSON array of notes: [{"id_user": int, "note": "text", "timestamp": "datetime"}]'' AFTER `notes`',
...
-- If notes exist, create a JSON array with one entry (id_user will be NULL for old notes)
UPDATE `sell_bill`
SET `notes_json` = CASE
  WHEN `notes` IS NOT NULL AND `notes` != '' THEN
    JSON_ARRAY(
      JSON_OBJECT(
        'id_user', 1,
        'note', `notes`,
        'timestamp', COALESCE(`time_created`, NOW())
      )
    )
  ELSE
    NULL
...
SET @sql = IF(@col_exists > 0, 
  'ALTER TABLE `sell_bill` DROP COLUMN `notes`',
...
SET @sql = IF(@col_exists > 0, 
  'ALTER TABLE `sell_bill` CHANGE COLUMN `notes_json` `notes` JSON DEFAULT NULL COMMENT ''JSON array of notes: [{"id_user": int, "note": "text", "timestamp": "datetime"}]''',
