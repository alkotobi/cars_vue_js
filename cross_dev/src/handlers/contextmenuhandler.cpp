...
public:
    ContextMenuHandler(WebViewWindow* window, MessageRouter* router)
        : window_(window), router_(router) {}
    ...
    nlohmann::json handle(const nlohmann::json& payload, const std::string& requestId) override {
        ...
        auto* cbData = new CallbackData{requestId, router_, window_, x, y};
