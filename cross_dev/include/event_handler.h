...
public:
    ...
    // Register callback for webview create window event (5-param: name, title, contentType, content, isSingleton)
    void onWebViewCreateWindow(std::function<void(const std::string& name, const std::string& title, WebViewContentType contentType, const std::string& content, bool isSingleton)> callback);

    // Overload for 3-param callback (title, contentType, content)
...
private:
    ...
    std::function<void(const std::string&, const std::string&, WebViewContentType, const std::string&, bool)> createWindowCallback_;
