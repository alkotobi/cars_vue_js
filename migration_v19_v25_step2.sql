...
-- This script assumes the early part of migration_v19_v25.sql
-- (banks/brands/buy_bill/cars_stock/chat_*/containers/priorities)
-- has already been applied on this DB.

-- 1) clients: adjust constraints (share_token column already added)
ALTER TABLE `clients`
...
-- 2) sell_bill: extend to v25 structure (keep existing notes type to avoid touching data)
...
-- 3) users: make role_id NOT NULL, align id_bank_account
ALTER TABLE `users`
  MODIFY `role_id` int(11) NOT NULL,
  MODIFY `id_bank_account` int(10) UNSIGNED DEFAULT NULL;

-- 4) New tables from v25 (all of them)
